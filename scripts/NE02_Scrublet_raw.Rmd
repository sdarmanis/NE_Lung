...
data_adult <- read.table(paste(dir,"input/GC_table_adult_NE_from_old.tsv",sep=""), check.names = F)
meta_adult_new <- read.table(paste(dir,"input/Metadata_table_adult_NE_from_old.tsv",sep=""), check.names = F)
```
...
temp.6 <- read.table(paste(dir,"input/NE_Lung_Exp.ID_6_doubletScore.csv", sep=""), sep=",")
colnames(temp.6) <- list.names$`6`
temp.7 <- read.table(paste(dir,"input/NE_Lung_Exp.ID_7_doubletScore.csv", sep=""), sep=",")
colnames(temp.7) <- list.names$`7`
temp.8 <- read.table(paste(dir,"input/NE_Lung_Exp.ID_8_doubletScore.csv", sep=""), sep=",")
...
temp.6 <- read.table(paste(dir,"input/NE_Lung_Exp.ID_6_doubletpred.csv", sep=""), sep=",")
colnames(temp.6) <- list.names$`6`
temp.7 <- read.table(paste(dir,"input/NE_Lung_Exp.ID_7_doubletpred.csv", sep=""), sep=",")
colnames(temp.7) <- list.names$`7`
temp.8 <- read.table(paste(dir,"input/NE_Lung_Exp.ID_8_doubletpred.csv", sep=""), sep=",")
colnames(temp.8) <- list.names$`8`
temp.all <- t(cbind(temp.6, temp.7,temp.8))
rm(list=c("temp.6", "temp.7","temp.8"))
# Add to Seurat metadata 
...
```
Compare to old doublet assignments 
```{r Table}
table(meta_adult_new$scrublet_doublet,meta_adult_new$sum_class)
```


Save data and metadata 
```{r }
write.table(data_adult, paste(dir,"input/GC_table_adult_NE_from_old.tsv",sep=""))
write.table(meta_adult_new, paste(dir,"input/Metadata_table_adult_NE_from_old.tsv",sep=""))
```
...
Save data and metadata 
```{r }
write.table(data_adult_ND, paste(dir,"input/GC_table_adult_NE_from_old_ND.tsv",sep=""))
write.table(meta_adult_new_ND, paste(dir,"input/Metadata_table_adult_NE_from_old_ND.tsv",sep=""))
```
